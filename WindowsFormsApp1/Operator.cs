...
namespace WindowsFormsApp1
{
    class Operator
    {
        ...
        public OleDbConnection getCon(string name)
        {
            string address = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + name + ".accdb;";
            OleDbConnection conn;
            try
            {
                conn = new OleDbConnection(address);//连接Access2007及以上版本
            }
            catch
            {
                conn = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source="+name+".mdb;");//连接Access2000及2003版本
            }
        ...
        public void ExecSQL(string sql,string name)
        {
            string address = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + name + ".accdb;";
            OleDbConnection conn = new OleDbConnection(address);//连接Access2007及以上版本
            conn.Open();//打开数据库连接
        ...
        public void ClearSystem(IntPtr handle, string str)
        {
            ...
            try
            {
                ...
                {
                    case "清空回收站":
                        Win32.SHEmptyRecycleBin(handle, 0, 7);//调用API函数清空回收站
                        break;
                    case "清空IE缓存区":
